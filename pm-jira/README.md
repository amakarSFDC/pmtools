...
**Previous Sprint — Velocity vs Commitment:** The most recently closed sprint on the board is always fetched — story points committed vs completed (`Closed` only) — and shown both as its own report section with a burn bar and as the final paragraph of the executive summary. "Committed" reflects only the Story/Bug issues present when the sprint **started**: the script calls JIRA's classic sprint-report endpoint (`GET {base_url}/rest/greenhopper/1.0/rapid/charts/sprintreport?rapidViewId={board}&sprintId={sprint_id}`) to get `issueKeysAddedDuringSprint` and excludes those issues' points from the committed total, even though they still count toward completed if closed. This avoids the mid-sprint-scope-creep trap where every added-and-closed issue inflates "committed" to match "completed," always showing ~100% regardless of how much scope was added after planning. This endpoint is undocumented/internal (not part of the public Agile REST API v3) — if Atlassian changes or removes it, `fetch_sprint_velocity()` falls back silently to treating all issues as original scope (the old behavior).
